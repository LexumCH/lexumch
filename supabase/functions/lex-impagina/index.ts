// supabase/functions/lex-impagina/index.ts  — Lexum CH
// ─────────────────────────────────────────────────────────────
...
// fermi, sintesi finale, descrizione delle fonti, NELLA LINGUA DELLA RISPOSTA
// (it/de/fr). Il testo della risposta NON viene riscritto: lo impagina il
// browser cosi' com'e' (src/lib/pdf).
//
// Costo misurato su IT ~1,7 centesimi a documento (Sonnet 5, effort low):
...
//
// Body:     { domanda: string, risposta: string, sezioni: string[], lingua?: 'it'|'de'|'fr' }
...
// Il tetto comprende il ragionamento; con effort low l'uscita misurata e' ~1.000 token.
...
const linguaSicura = (l: unknown): Lingua => (l === 'de' || l === 'fr') ? l : 'it'
const MSG: Record<Lingua, { limite: string; errore: string }> = {
  it: { limite: `Hai raggiunto il limite di ${LIMITE_GIORNALIERO} PDF curati al giorno.`, errore: 'Il servizio non ha risposto in tempo.' },
  de: { limite: `Sie haben das Limit von ${LIMITE_GIORNALIERO} aufbereiteten PDFs pro Tag erreicht.`, errore: 'Der Dienst hat nicht rechtzeitig geantwortet.' },
  fr: { limite: `Vous avez atteint la limite de ${LIMITE_GIORNALIERO} PDF mis en forme par jour.`, errore: "Le service n'a pas répondu à temps." },
}

const PROMPT = `Sei l'impaginatore editoriale di Lexum, piattaforma giuridica per professionisti svizzeri. Ricevi la DOMANDA di un utente, la RISPOSTA gia' scritta da Lex, l'assistente giuridico di Lexum, e l'elenco numerato delle sue SEZIONI. Il testo della risposta verra' impaginato cosi' com'e': tu scrivi solo gli elementi editoriali attorno, NELLA STESSA LINGUA DELLA RISPOSTA (italiano, tedesco o francese). Restituisci SOLO un oggetto JSON con questo schema:
{
  "lingua": "it, de oppure fr: la lingua della risposta",
...
- Mantieni il registro della risposta: se da' del Lei, del Sie o del vous, continua cosi'.
- Le fonti sono SOLO quelle citate nella risposta, norme e sentenze, nell'ordine in cui compaiono, nel formato svizzero usato dalla risposta (per esempio art. 41 CO; DTF 145 III 72; sentenza TF 4A_123/2024).
Testo in linea: **grassetto** per i passaggi chiave, con misura; [[...]] attorno a ogni citazione di norma o sentenza, per esempio [[art. 41 CO]].
...
function normalizza(g: any, nSezioni: number, linguaUi: Lingua) {
  return {
    lingua: (g?.lingua === 'it' || g?.lingua === 'de' || g?.lingua === 'fr') ? g.lingua : linguaUi,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    lingua = linguaSicura(body?.lingua)
    ...
    if (!risposta.trim()) return json(400, { ok: false, error: MSG[lingua].errore })
    if (risposta.length > MAX_CARATTERI_RISPOSTA) return json(400, { ok: false, error: MSG[lingua].errore })
    ...
    if ((count ?? 0) >= LIMITE_GIORNALIERO) return json(429, { ok: false, error: MSG[lingua].limite })
    ...
    if (!r.ok || !d) throw new Error(`Anthropic ${r.status}: ${JSON.stringify(d?.error ?? null).slice(0, 200)}`)
    ...
    const editoriale = normalizza(JSON.parse(grezzo.slice(grezzo.indexOf('{'), grezzo.lastIndexOf('}') + 1)), sezioni.length, lingua)
    ...
    await logLexCall({
      ...
      metadati: { stop_reason: d.stop_reason ?? null, caratteri_risposta: risposta.length, sezioni: sezioni.length, lingua: editoriale.lingua },
    })
    ...
    return json(500, { ok: false, error: MSG[lingua].errore })
  }
})
