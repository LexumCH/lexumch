...
export default function DocumentiPortale({ clienteId }) {
    const { t, i18n } = useTranslation('comp_documenti_portale')
    ...
    async function rimuovi(doc) {
        if (!confirm(t('conferma_rimuovi', { nome: doc.nome }))) return
    ...
    return (
        <div className="bg-slate border border-white/5 p-5 space-y-4">
            <div className="flex items-center gap-2">
                ...
                <p className="section-label !mb-0">{t('titolo')}</p>
            </div>
            ...
            <div className="flex items-center gap-3">
                    ...
                    <div className={`border border-dashed p-3 text-center transition-all ${file ? 'border-salvia/30 bg-salvia/5' : 'border-white/15 hover:border-oro/30'}`}>
                        {file
                            ...
                            : <div className="flex items-center justify-center gap-2 text-nebbia/30">
                                <Upload size={15} /><span className="font-body text-sm">{t('seleziona_file')}</span>
                            </div>}
                    </div>
                ...
                <button onClick={condividi} disabled={!file || uploading}
                    ...
                    {uploading ? <span className="animate-spin w-4 h-4 border-2 border-petrolio border-t-transparent rounded-full inline-block" /> : t('condividi')}
                </button>
            </div>
            {errore && (
            ...
            {loading ? (
                ...
                <p className="font-body text-sm text-nebbia/25 text-center py-4">{t('vuoto')}</p>
            ) : (
                            ...
                            <div key={d.id} className="flex items-center gap-3 p-2.5 bg-petrolio/40 border border-white/5">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="font-body text-[11px] text-nebbia/35">
                                        {new Date(d.created_at).toLocaleDateString(dateLocale)} · {formatSize(d.dimensione)}
                                    </p>
                                </div>
                                <span className={`font-body text-[10px] px-1.5 py-0.5 border shrink-0 ${daCliente ? 'bg-salvia/10 border-salvia/25 text-salvia/80' : 'bg-oro/10 border-oro/25 text-oro/80'}`}>
                                    {daCliente ? t('badge.dal_cliente') : t('badge.dallo_studio')}
                                </span>
                                <button onClick={() => apri(d)} title={t('azioni.visualizza')}
                                ...
                                <button onClick={() => rimuovi(d)} title={t('azioni.rimuovi')}
