...
import { proiezioneLiquidita, MESI_ABBR } from '@/lib/calcoloLiquidita'
...
export default function PianificazioneLiquidita({ clienteId, mandatoId = null, refreshTrigger = 0 }) {
    ...
    async function salvaSaldo() {
        ...
        if (importoEdit === '' || isNaN(n)) { setErrore('Inserisci un importo valido.'); return }
        if (!dataEdit) { setErrore('Inserisci la data del saldo.'); return }
        ...
        <div className="bg-slate border border-white/5 p-6 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                {editing ? (
                    <div className="flex items-end gap-2 flex-wrap">
                        <div>
                            <label className="block font-body text-[10px] text-nebbia/30 uppercase tracking-widest mb-1">Saldo cassa (CHF)</label>
                    ...
                    <button onClick={apriEditor}
                        ...
                        <span className="font-body text-sm text-nebbia">
                            {saldo ? fmtCHF(saldo.importo) : 'Imposta saldo cassa'}
                        </span>
                        {saldo && <span className="font-body text-[11px] text-nebbia/30">al {new Date(saldo.data).toLocaleDateString('it-CH')}</span>}
            ...
            {tabellaMancante && (
                <div className="flex items-center gap-2 text-amber-400 text-xs font-body p-3 bg-amber-900/10 border border-amber-500/20">
                    ...
                    Tabella <code className="font-mono">saldi_cassa</code> non ancora creata: esegui la migration per salvare il saldo e proiettare la liquidità.
                </div>
            )}
            {errore && (
            ...
            {loading ? (
                ...
                <div className="bg-petrolio/40 border border-white/5 p-8 text-center flex flex-col items-center justify-center min-h-[200px]">
                    ...
                    <p className="font-body text-sm text-nebbia/40">Imposta il saldo di cassa iniziale per vedere la proiezione a 12 mesi.</p>
                    <p className="font-body text-xs text-nebbia/25 mt-1">La proiezione userà i movimenti previsti, gli effettivi futuri e i salari.</p>
                </div>
            ) : (
                <>
                    {/* KPI */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                        <div className="bg-slate border border-white/10 p-4">
                            <p className="font-body text-[10px] text-nebbia/30 uppercase tracking-widest mb-1.5">Saldo oggi</p>
                    ...
                    {proj.primaSottoZero && (
                        <div className="flex items-start gap-2 text-red-400 text-sm font-body p-3 bg-red-900/10 border border-red-500/25">
                            ...
                            <span>
                                Tensione di cassa prevista: il saldo va sotto zero a <span className="font-medium capitalize">{MESI_ABBR[proj.primaSottoZero.mese]} {proj.primaSottoZero.anno}</span> ({fmtCHF(proj.primaSottoZero.saldoFine)}). Pianifica un incasso o riduci i costi previsti.
                            </span>
                        </div>
                    )}
                            ...
                            {proj.buckets.map((b, i) => {
                                ...
                                return (
                                    <div key={i} className="flex-1 flex flex-col items-center"
                                        title={`${MESI_ABBR[b.mese]} ${b.anno}: ${fmtCHF(b.saldoFine)} (netto ${fmtCHF(b.netto)})`}>
                        ...
                        <table className="w-full">
                            <thead className="sticky top-0 bg-slate">
                                <tr className="border-b border-white/5">
                                    {['Mese', 'Entrate', 'Uscite', 'Salari', 'Netto', 'Saldo fine'].map(h => (
                    ...
                    <p className="font-body text-[11px] text-nebbia/25">
                        Proiezione: saldo noto + entrate/uscite previste (espanse per ricorrenza) + effettivi futuri − salari. Il mese in corso esclude i salari (già nel saldo). Aggiorna il saldo regolarmente per tenerla accurata.
                    </p>
                </>
            )}
        </div>
