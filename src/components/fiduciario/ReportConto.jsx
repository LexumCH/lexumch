// src/components/fiduciario/ReportConto.jsx
//
// Reporting del conto economico (consuntivo, solo movimenti EFFETTIVI + salari):
//   A) Andamento mensile dell'anno — entrate vs uscite, mese per mese
//   B) Composizione per categoria — dove vanno i soldi (uscite) e da dove
//      arrivano (entrate), con gli stipendi come voce di uscita
//   C) Confronto pluriennale — entrate/uscite/saldo degli ultimi anni
//
// Tutto su dati esistenti: nessun nuovo schema. Grafici CSS, zero dipendenze.
//
// Props:
...
import {
    fmtCHF, salariMeseAttivi, bonusDelMese, salariProRataAnno, bonusDellAnnoX,
...
export default function ReportConto({ clienteId, mandatoId = null, anno = null, refreshTrigger = 0 }) {
    ...
    async function carica() {
        ...
        const [{ data: mov, error }, { data: dip }, { data: bon }, { data: cli }, { data: mand }] = await Promise.all([
            q,
        ...
        if (error) { setTabellaMancante(error.code === '42P01' || /movimenti/i.test(error.message ?? '')); setMovimenti([]) }
        else { setTabellaMancante(false); setMovimenti(mov ?? []) }
        setDipendenti(dip ?? [])
        setBonus(bon ?? [])
    ...
    // Solo consuntivo (i previsti sono budget, vivono in BudgetScostamenti)
    ...
    const mesi = Array.from({ length: 12 }, (_, mo) => {
        ...
        const salari = salariMeseAttivi(dipendenti, mo, annoSel) + bonusDelMese(bonus, mo, annoSel)
        const uscite = costi + salari
        return { mo, entrate, uscite, netto: entrate - uscite }
    })
    const maxMese = Math.max(1, ...mesi.flatMap(m => [m.entrate, m.uscite]))

    // ─── B) Composizione per categoria (annoSel) ───
                                ...
                                <tbody>
                                    {perAnno.map(a => (
                                        <tr key={a.anno} className="border-b border-white/5 last:border-0">
                                            ...
                                            <td className="px-3 py-2 text-right font-body text-xs text-salvia/80">{a.entrate ? fmtCHF(a.entrate) : '—'}</td>
                                            <td className="px-3 py-2 text-right font-body text-xs text-oro/70">{a.uscite ? fmtCHF(a.uscite) : '—'}</td>
                                            <td className={`px-3 py-2 text-right font-display text-sm ${a.saldo >= 0 ? 'text-salvia' : 'text-red-400'}`}>{fmtCHF(a.saldo)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        return (
                            <div key={i}>
                                <div className="flex items-center justify-between mb-1 gap-2">
                                    ...
                                    <span className="font-body text-xs text-nebbia/40 shrink-0">{fmtCHF(r.val)} <span className="text-nebbia/25">({pct}%)</span></span>
                                </div>
