// src/components/fiduciario/Contabilita.jsx
//
// Contabilità in partita doppia (mastro unico) — Fase A:
//   - Piano dei conti per cliente (inizializzabile dallo standard KMU, editabile)
//   - Libro giornale: registrazioni con righe dare/avere bilanciate (incl. conti IVA)
//
// I report Bilancio / Conto economico sono la Fase B (dai saldi dei conti).
//
// Props:
...
import { righeSeedPiano, NOME_CLASSE } from '@/lib/pianoContiKMU'
...
export default function Contabilita({ clienteId, mandatoId = null }) {
    const { t, i18n } = useTranslation('comp_fid_contabilita')
    ...
    const [vista, setVista] = useState('giornale')   // 'giornale' | 'conti'
    ...
    async function carica() {
        ...
        const [{ data: pc, error: errPc }, { data: reg }] = await Promise.all([
        ...
        if (errPc && (errPc.code === '42P01' || /piano_conti/i.test(errPc.message ?? ''))) {
            setTabellaMancante(true); setConti([]); setRegistrazioni([]); setLoading(false); return
        }
    ...
    async function inizializzaPiano() {
        ...
        if (!user) { setErrore(t('errori.sessione_scaduta')); setInizializzando(false); return }
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-body text-[10px] px-2 py-0.5 bg-petrolio border border-white/10 text-nebbia/40 uppercase tracking-wider">{t('badge_partita_doppia')}</span>
                </div>
                {conti.length > 0 && (
                    <div className="flex items-center gap-2">
                        <div className="flex border border-white/10">
                            {[['giornale', t('tab.giornale')], ['conti', t('tab.conti')], ['report', t('tab.report')], ['imputazioni', t('tab.imputazioni')]].map(([v, l]) => (
                        ...
                        {vista === 'giornale' && (
                            <button onClick={() => setForm(true)}
                                ...
                                <Plus size={11} /> {t('azioni.nuova_registrazione')}
                            </button>
                        )}
                    </div>
                )}
            ...
            {tabellaMancante && (
                <div className="flex items-center gap-2 text-amber-400 text-xs font-body p-3 bg-amber-900/10 border border-amber-500/20">
                    <AlertCircle size={14} className="shrink-0" /> <span><Trans i18nKey="tabella_mancante" t={t} components={{ code: <code className="font-mono" /> }} /></span>
                </div>
            )}
            {errore && (
            ...
            {loading ? (
            ...
            ) : tabellaMancante ? null : conti.length === 0 ? (
                <div className="bg-petrolio/40 border border-white/5 p-10 text-center">
                    ...
                    <p className="font-body text-sm text-nebbia/40">{t('vuoto_piano.titolo')}</p>
                    <p className="font-body text-xs text-nebbia/25 mt-1 mb-4">{t('vuoto_piano.sottotitolo')}</p>
                    <button onClick={inizializzaPiano} disabled={inizializzando}
                        ...
                        {inizializzando ? <Loader2 size={14} className="animate-spin" /> : <ListTree size={14} />} {t('vuoto_piano.inizializza')}
                    </button>
                </div>
            ) : vista === 'report' ? (
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <span className="font-body text-sm text-nebbia">{new Date(r.data).toLocaleDateString(dateLocale)}</span>
                                        <span className="font-body text-sm text-nebbia/70 truncate">{r.descrizione}</span>
                                        {r.tipo !== 'ordinaria' && (
                                            <span className="font-body text-[10px] px-1.5 py-0.5 border border-white/10 text-nebbia/40 uppercase tracking-wider">{t(`tipo.${r.tipo}`, r.tipo)}</span>
                                        )}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                    <span className="font-display text-sm text-oro">{fmtCHF(totaleRiga(r.righe))}</span>
                                    <button onClick={() => setDaEliminare(r)} title={t('azioni.elimina_registrazione')}
                            ...
                            <div className="mt-2 border-t border-white/5 pt-2 space-y-0.5">
                                {(r.righe ?? []).map(rg => (
                                    <div key={rg.id} className="grid grid-cols-[1fr_auto_auto] gap-3 font-body text-xs">
                                        ...
                                        <span className="text-right text-nebbia/70 w-28">{Number(rg.dare) ? fmtCHF(rg.dare) : ''}</span>
                                        <span className="text-right text-nebbia/70 w-28">{Number(rg.avere) ? fmtCHF(rg.avere) : ''}</span>
                                    </div>
                                ))}
                                <div className="grid grid-cols-[1fr_auto_auto] gap-3 font-body text-[10px] text-nebbia/30 uppercase tracking-widest pt-1">
                                    <span></span><span className="text-right w-28">{t('colonne.dare')}</span><span className="text-right w-28">{t('colonne.avere')}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="bg-slate border border-red-500/30 w-full max-w-md" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            <p className="font-body text-sm text-nebbia/60">
                                <Trans i18nKey="elimina.conferma" t={t}
                                    values={{ descrizione: daEliminare.descrizione, data: new Date(daEliminare.data).toLocaleDateString(dateLocale) }}
                                    components={{ d: <span className="text-nebbia" /> }} />
                            </p>
                            <div className="flex gap-2">
                                <button onClick={() => setDaEliminare(null)} className="font-body text-sm text-nebbia/60 hover:text-nebbia border border-white/10 px-4 py-2.5">{t('azioni.annulla')}</button>
                                <button onClick={elimina} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-red-500/15 border border-red-500/40 text-red-400 font-body text-sm hover:bg-red-500/25 transition-colors"><Trash2 size={14} /> {t('azioni.elimina')}</button>
                            </div>
                        </div>
                    </div>
                </div>
...
function PianoContiVista({ conti }) {
    const { t } = useTranslation('comp_fid_contabilita')
            ...
            {classi.map(cl => (
                <div key={cl} className="bg-slate border border-white/5">
                    <div className="px-4 py-2 border-b border-white/5">
                        <p className="section-label !m-0">{cl} — {NOME_CLASSE[cl] ?? t('classe_altro')}</p>
                    </div>
                    <div className="divide-y divide-white/5">
                        {conti.filter(c => c.classe === cl).map(c => (
                            <div key={c.id} className="flex items-center gap-3 px-4 py-1.5">
                                ...
                                {!c.attivo && <span className="font-body text-[10px] text-nebbia/25">{t('disattivo')}</span>}
                            </div>
                        ))}
                    </div>
                </div>
...
function FormRegistrazione({ clienteId, mandatoId, conti, onClose, onSaved }) {
    ...
    async function salva() {
        if (!descrizione.trim()) { setErrore(t('errori.descrizione_obbligatoria')); return }
        if (!bilanciato) { setErrore(t('errori.non_bilanciata')); return }
        ...
        setSalvando(false)
        if (errRighe) {
            // rollback testata se le righe falliscono (es. sbilanciamento dal trigger)
            ...
            <div className="bg-slate border border-white/10 w-full max-w-2xl shadow-2xl flex flex-col max-h-[calc(100vh-2rem)]" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="font-display text-lg text-nebbia">{t('azioni.nuova_registrazione')}</p>
                    </div>
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1">
                    <div className="grid grid-cols-[140px_1fr_150px] gap-3">
                        <div>
                            <label className="block font-body text-[10px] text-nebbia/40 uppercase tracking-widest mb-1">{t('form.data')}</label>
                            <input type="date" value={data} onChange={e => setData(e.target.value)} className={inputCls} />
                        </div>
                        <div>
                            <label className="block font-body text-[10px] text-nebbia/40 uppercase tracking-widest mb-1">{t('form.descrizione')}</label>
                            <input value={descrizione} onChange={e => setDescrizione(e.target.value)} placeholder={t('form.descrizione_placeholder')} className={inputCls} autoFocus />
                        </div>
                        <div>
                            <label className="block font-body text-[10px] text-nebbia/40 uppercase tracking-widest mb-1">{t('form.tipo')}</label>
                            <select value={tipo} onChange={e => setTipo(e.target.value)} className={inputCls}>
                                <option value="ordinaria">{t('tipo.ordinaria')}</option>
                                <option value="apertura">{t('tipo.apertura')}</option>
                                <option value="rettifica">{t('tipo.rettifica')}</option>
                                <option value="chiusura">{t('tipo.chiusura')}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="grid grid-cols-[1fr_120px_120px_28px] gap-2 font-body text-[10px] text-nebbia/30 uppercase tracking-widest mb-1.5 px-1">
                            <span>{t('colonne.conto')}</span><span className="text-right">{t('colonne.dare')}</span><span className="text-right">{t('colonne.avere')}</span><span></span>
                        </div>
                            ...
                            {righe.map((r, i) => (
                                <div key={i} className="grid grid-cols-[1fr_120px_120px_28px] gap-2 items-center">
                                    <select value={r.conto_id} onChange={e => setRiga(i, 'conto_id', e.target.value)} className={inputCls}>
                                        <option value="">{t('form.scegli_conto')}</option>
                        ...
                        <button onClick={addRiga} className="mt-2 flex items-center gap-1.5 font-body text-xs text-nebbia/50 hover:text-oro transition-colors">
                            <Plus size={12} /> {t('form.aggiungi_riga')}
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-[1fr_120px_120px_28px] gap-2 border-t border-white/10 pt-2 font-body text-sm">
                        <span className={`${bilanciato ? 'text-salvia' : 'text-nebbia/40'} flex items-center gap-1.5`}>
                            {bilanciato ? <><Check size={13} /> {t('bilancio.bilanciata')}</> : diff !== 0 ? t('bilancio.sbilancio', { importo: fmtCHF(Math.abs(diff)) }) : t('bilancio.inserisci_righe')}
                        </span>
                        <span className="text-right text-nebbia">{fmtCHF(totDare)}</span>
                        <span className="text-right text-nebbia">{fmtCHF(totAvere)}</span>
                        <span></span>
                    </div>
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/5 shrink-0">
                    <button onClick={onClose} disabled={salvando} className="font-body text-sm text-nebbia/60 hover:text-nebbia px-4 py-2 disabled:opacity-40">{t('azioni.annulla')}</button>
                    <button onClick={salva} disabled={salvando || !bilanciato}
                        ...
                        {salvando ? <><Loader2 size={14} className="animate-spin" /> {t('azioni.salvataggio')}</> : t('azioni.registra')}
                    </button>
                </div>
            </div>
        </div>
