        ...
        <div className="space-y-5">
                ...
                {conti.length > 0 && (
                    <div className="flex items-center gap-2">
                        <div className="flex border border-white/10">
                            {[['giornale', 'Libro giornale'], ['conti', 'Piano dei conti'], ['report', 'Report'], ['imputazioni', 'Imputazioni']].map(([v, l]) => (
            ...
            {tabellaMancante && (
                <div className="flex items-center gap-2 text-amber-400 text-xs font-body p-3 bg-amber-900/10 border border-amber-500/20">
                    <AlertCircle size={14} className="shrink-0" /> Tabelle contabili non ancora create: esegui la migration <code className="font-mono">contabilita_fase_a.sql</code>.
                </div>
            )}
            {errore && (
            ...
            {loading ? (
                ...
                <div className="bg-petrolio/40 border border-white/5 p-10 text-center">
                    ...
                    <p className="font-body text-sm text-nebbia/40">Nessun piano dei conti per questo cliente.</p>
                    <p className="font-body text-xs text-nebbia/25 mt-1 mb-4">Inizializza il piano standard svizzero per PMI (poi personalizzabile).</p>
                ...
                <div className="space-y-2">
                    {registrazioni.length === 0 ? (
                        <div className="bg-petrolio/40 border border-white/5 p-8 text-center font-body text-xs text-nebbia/30">
                            Nessuna registrazione. Usa “Nuova registrazione” per la prima scrittura (o per i saldi d'apertura).
                        </div>
                    ) : registrazioni.map(r => (
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <span className="font-body text-sm text-nebbia">{new Date(r.data).toLocaleDateString('it-CH')}</span>
                                        <span className="font-body text-sm text-nebbia/70 truncate">{r.descrizione}</span>
                                        {r.tipo !== 'ordinaria' && (
                                            <span className="font-body text-[10px] px-1.5 py-0.5 border border-white/10 text-nebbia/40 uppercase tracking-wider">{r.tipo}</span>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 z-50 bg-petrolio/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setDaEliminare(null)}>
                    <div className="bg-slate border border-red-500/30 w-full max-w-md" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-4">
                            <p className="font-body text-sm text-nebbia/60">Eliminare <span className="text-nebbia">{daEliminare.descrizione}</span> del {new Date(daEliminare.data).toLocaleDateString('it-CH')}? Operazione irreversibile.</p>
...
function FormRegistrazione({ clienteId, mandatoId, conti, onClose, onSaved }) {
    ...
    async function salva() {
        ...
        if (!bilanciato) { setErrore('La scrittura deve bilanciare (Σ dare = Σ avere) con almeno 2 righe.'); return }
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1">
                    <div className="grid grid-cols-[140px_1fr_150px] gap-3">
                        ...
                        <div>
                            ...
                            <input value={descrizione} onChange={e => setDescrizione(e.target.value)} placeholder="es. Fattura cliente X / Pagamento affitto" className={inputCls} autoFocus />
                        </div>
                        <div>
                    ...
                    <div className="grid grid-cols-[1fr_120px_120px_28px] gap-2 border-t border-white/10 pt-2 font-body text-sm">
                        <span className={`${bilanciato ? 'text-salvia' : 'text-nebbia/40'} flex items-center gap-1.5`}>
                            {bilanciato ? <><Check size={13} /> Bilanciata</> : diff !== 0 ? `Sbilancio ${fmtCHF(Math.abs(diff))}` : 'Inserisci le righe'}
                        </span>
