...
export default function ChatWidget() {
    const { t } = useTranslation('comp_chat_widget')
    ...
    return (
        <>
                ...
                <div className="fixed bottom-20 right-5 w-80 sm:w-96 bg-slate border border-white/10 shadow-2xl shadow-petrolio/50 z-50 overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-petrolio/80">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-body text-sm font-medium text-nebbia">{t('header.title')}</span>
                        </div>
                        <button
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                {t('cta.register')} <ArrowRight size={13} />
                            </Link>
                            <Link
                                to="/login"
                                onClick={() => setAperto(false)}
            ...
            >
                {aperto
                    ? <><X size={15} /> {t('button.close')}</>
                    : <><MessageSquare size={15} /> {t('button.open')}</>
                }
            </button>
        </>
    )
