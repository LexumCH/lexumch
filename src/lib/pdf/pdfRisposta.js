// src/lib/pdf/pdfRisposta.js  — Lexum CH
// ─────────────────────────────────────────────────────────────
...
// fonti li scrive Sonnet nella lingua della risposta (edge lex-impagina,
// ~1,7 centesimi, nessun credito). Se Sonnet non risponde il PDF esce lo
// stesso, con le sole parti di Lex.
...
async function chiediEditoriale({ domanda, risposta, sezioni, lingua }) {
    const token = await getAccessToken()
    const res = await fetch(`${supabaseUrl}/functions/v1/lex-impagina`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ domanda, risposta, sezioni, lingua }),
    })
...
const nomeFile = (titolo) =>
    `LEXUM - ${(titolo || 'Lex AI').replace(/[\\/:*?"<>|]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 80)}.pdf`

/**
 * Crea il PDF di una risposta.
 * @returns {{ blob: Blob, nome: string, senzaParti: string|null }} senzaParti: il motivo,
 *          se il PDF e' uscito senza le parti di Sonnet (il chiamante lo traduce e lo mostra)
 */
export async function creaPdfRisposta({ domanda, risposta }) {
    const lingua = (i18n?.language ?? 'it').slice(0, 2)
    const base = modelloMeccanico({ domanda, risposta, lingua })
    let modello = base
    let senzaParti = null
    try {
        const editoriale = await chiediEditoriale({ domanda, risposta, lingua, sezioni: base.sezioni.map((s) => s.titolo) })
        modello = unisciEditoriale(base, editoriale)
    } catch (e) {
        senzaParti = sanitizzaErrore(e) ?? ''
    }
    ...
    const blob = await Promise.race([
        ...
        new Promise((_, ko) => setTimeout(() => ko(new Error('timeout_pdf')), 30000)),
    ])
    return { blob, nome: nomeFile(modello.titolo), senzaParti }
}

/** Crea il PDF e lo fa scaricare al browser. */
export async function scaricaPdfRisposta(parametri) {
    const { blob, nome, senzaParti } = await creaPdfRisposta(parametri)
