...
// PDF, senza AI (25/09/2026). Lexum CH: it/de/fr, le etichette del documento
// le sceglie l'impaginatore dalla lingua del modello. Legge solo la struttura che la risposta ha gia':
...
const TITOLO_GENERICO = { it: 'Analisi giuridica', de: 'Rechtsanalyse', fr: 'Analyse juridique' }
const SEZIONE_UNICA = { it: 'La risposta di Lex', de: 'Die Antwort von Lex', fr: 'La réponse de Lex' }
const linguaSicura = (l) => (l === 'de' || l === 'fr') ? l : 'it'
...
function titoloDaDomanda(domanda, lingua) {
    const prima = (domanda ?? '').trim().split(/(?<=[.?!])\s/)[0] ?? ''
    return prima.length > 0 && prima.length <= 80 ? prima.replace(/[.]$/, '') : TITOLO_GENERICO[lingua]
}
...
export function modelloMeccanico({ domanda, risposta, lingua: linguaUi }) {
    const lingua = linguaSicura(linguaUi)
    ...
    if (sezioni.length === 0) sezioni.push({ titolo: SEZIONE_UNICA[lingua], blocchi: premessa.splice(0) })

    // Chiusura "In sintesi: ...", "Zusammenfassend ...", "En résumé ...": va nel riquadro finale.
    let sintesi
    const ultimi = sezioni[sezioni.length - 1].blocchi
    const coda = ultimi[ultimi.length - 1]
    const apertura = /^\*{0,2}(in sintesi|in conclusione|in definitiva|zusammenfassend|zusammengefasst|fazit|en résumé|en conclusion|en bref|en somme)\*{0,2}\s*[:,.]\s*/i
    ...
    return {
        lingua,
        titolo: titoloDaDomanda(domanda, lingua),
        sottotitolo: sezioni.slice(0, 4).map((s) => s.titolo).join(' · '),
        quesito: { etichetta: 'quesito', testo: (domanda ?? '').trim() },
        premessa,
        sezioni,
        sintesi,
...
// nella lingua della risposta: le etichette del PDF seguono quella lingua.
// Se la risposta chiudeva gia' con "In sintesi", resta quella di Lex.
export function unisciEditoriale(base, ed) {
    if (!ed) return base
    return {
        ...base,
        lingua: ed.lingua ? linguaSicura(ed.lingua) : base.lingua,
        titolo: ed.titolo || base.titolo,
        sottotitolo: ed.sottotitolo || base.sottotitolo,
        quesito: ed.quesito ? { etichetta: 'quesito_sintesi', testo: ed.quesito } : base.quesito,
