...
// Lexum CH: le etichette fisse escono nella lingua del modello (it/de/fr).
//
// MODELLO
// {
//   lingua: 'it' | 'de' | 'fr',
//   titolo, sottotitolo?,
//   quesito: { etichetta: 'quesito' | 'quesito_sintesi', testo },
//   premessa?: Blocco[],
...
const ETICHETTE = {
    it: {
        intestazione: 'Lex AI · Analisi giuridica', quesito: 'Il quesito', quesito_sintesi: 'Il quesito, in sintesi',
        premessa: 'Premessa', punti_fermi: 'In sintesi — i punti fermi', sintesi: 'In sintesi',
        fonti: 'Fonti richiamate', principio: 'Principio richiamato', piede: 'Analisi generata con Lex AI',
        nota: 'Documento generato con Lex AI sulle fonti normative e giurisprudenziali indicizzate nella banca dati Lexum. Non costituisce un parere legale né sostituisce la valutazione professionale del professionista incaricato.',
    },
    de: {
        intestazione: 'Lex AI · Rechtsanalyse', quesito: 'Die Frage', quesito_sintesi: 'Die Frage, zusammengefasst',
        premessa: 'Vorbemerkung', punti_fermi: 'Zusammengefasst — die Eckpunkte', sintesi: 'Zusammenfassung',
        fonti: 'Zitierte Quellen', principio: 'Zitierter Grundsatz', piede: 'Mit Lex AI erstellte Analyse',
        nota: 'Mit Lex AI erstelltes Dokument auf Grundlage der in der Lexum-Datenbank indexierten Rechtsquellen und Rechtsprechung. Es stellt kein Rechtsgutachten dar und ersetzt nicht die fachliche Beurteilung der beauftragten Fachperson.',
    },
    fr: {
        intestazione: 'Lex AI · Analyse juridique', quesito: 'La question', quesito_sintesi: 'La question, en bref',
        premessa: 'Préambule', punti_fermi: 'En bref — les points clés', sintesi: 'En bref',
        fonti: 'Sources citées', principio: 'Principe cité', piede: 'Analyse générée avec Lex AI',
        nota: "Document généré avec Lex AI à partir des sources normatives et jurisprudentielles indexées dans la banque de données Lexum. Il ne constitue pas un avis juridique et ne remplace pas l'appréciation du professionnel mandaté.",
    },
}
...
function blocco(b, dopoPrincipio, L = ETICHETTE.it) {
    switch (b.tipo) {
        case 'paragrafo':
        ...
        case 'principio':
            return {
                stack: [
                    ...
                    spaziato(b.etichetta ?? L.principio, { color: C.oro }),
...
function puntiFermi(m, contenuto, L) {
    if (!m.punti_fermi?.length) return
    contenuto.push(filetto([0, 10, 0, 10]))
    contenuto.push(spaziato(L.punti_fermi, { margin: [0, 0, 0, 8] }))
    contenuto.push({
...
function sintesiEFonti(m, contenuto, L) {
    if (m.sintesi) {
        contenuto.push(riquadro([
            spaziato(L.sintesi, { color: C.oro, margin: [0, 0, 0, 6] }),
    ...
    if (m.fonti?.length) {
        contenuto.push({
            stack: [
                spaziato(L.fonti, { margin: [0, 8, 0, 6] }),
                {
                    table: {
                        widths: [150, '*'],
                        body: m.fonti.map((f) => [
    ...
    contenuto.push({ text: L.nota, fontSize: 7.3, color: C.grigioScuro, margin: [0, 14, 0, 0] })
}
...
export function definizioneDocumento(m, { logo } = {}) {
    const contenuto = []
    const L = ETICHETTE[m.lingua] ?? ETICHETTE.it
    ...
    contenuto.push({ ...spaziato(L.intestazione), alignment: 'center', margin: [0, 0, 0, 16] })
    ...
    // Quesito e premessa
    contenuto.push(riquadro([
        spaziato(L[m.quesito.etichetta] ?? L.quesito, { color: C.oro, margin: [0, 0, 0, 6] }),
        { text: m.quesito.testo, font: 'Cormorant', italics: true, fontSize: 12, color: C.oroChiaro, lineHeight: 1.2 },
    ], C.oro))
    if (m.premessa?.length) {
        contenuto.push(riquadro([
            spaziato(L.premessa, { color: C.salvia, bold: true, margin: [0, 0, 0, 6] }),
            ...m.premessa.map((b) => ({ ...blocco(b, false, L), fontSize: 8.8 })),
        ], C.salvia))
    }
    ...
    m.sezioni.forEach((s, i) => {
        ...
        s.blocchi.forEach((b, j) => {
            ...
            const r = blocco(b, b.tipo === 'principio' && s.blocchi[j - 1]?.tipo === 'principio', L)
            if (r) contenuto.push(r)
        })
        if (i === 0) puntiFermi(m, contenuto, L)
    })

    sintesiEFonti(m, contenuto, L)
    ...
    return {
        ...
        footer: (pagina, totale) => ({
            stack: [
                ...
                {
                    columns: [
                        { text: [{ text: 'LEXUM', bold: true, color: C.oro, characterSpacing: 1.4 }, { text: `  ·  ${L.piede}  ·  lexum.ch`, color: C.grigioScuro }], fontSize: 7 },
