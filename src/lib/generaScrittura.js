...
//
// Partita doppia — Fase C: genera la scrittura da un movimento effettivo
// usando la sua imputazione (mapping categoria→conto). IVA inclusa nel lordo.
//
//   ENTRATA:  Dare Cassa/Banca (lordo) ; Avere Ricavo (netto) + IVA dovuta (2200)
//   USCITA:   Dare Costo (netto) + Imposta precedente (1170) ; Avere Cassa/Banca (lordo)
//
...
export function righeDaMovimento(movimento, mapping, contiByNumero = {}) {
    ...
    if (movimento.tipo === 'entrata') {
        ...
        if (iva > 0) {
            ...
            if (!c) return { errore: 'Conto IVA dovuta (2200) assente nel piano' }
        ...
        if (iva > 0) {
            ...
            if (!c) return { errore: 'Conto Imposta precedente (1170) assente nel piano' }
